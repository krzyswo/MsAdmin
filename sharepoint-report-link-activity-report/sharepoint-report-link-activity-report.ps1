...
While($true)
{
 ...
 Try
 {
  ...
  {
   Write-Host `
Anonymous activity report can be retrieved only for past 90 days. Please select a date after $MaxStartDate` -ForegroundColor Red
   return
  }
 }
 Catch
...
Write-Host `
Retrieving anonymous link events from $StartDate to $EndDate...`
...
{ 
 ...
 Write-Progress -Activity "`
     Retrieving anonymous link activities from $CurrentStart to $CurrentEnd..`"`n" Processed audit record count: $ProcessedAuditCount"
...
{
 Write-Host `
The output file contains $OutputEvents audit records`
 if((Test-Path -Path $OutputCSV) -eq "True") 
 {
  Write-Host `
 The Output file availble in: -NoNewline -ForegroundColor Yellow`
  Write-Host $OutputCSV 
  Write-Host `
~~ Script prepared by AdminDroid Community ~~` -ForegroundColor Green
