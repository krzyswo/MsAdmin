...
Write-Host `
Auditing SharePoint group membership changes from $StartDate to $EndDate...
...
while($true)
{
 ...
 Search-UnifiedAuditLog -StartDate $CurrentStart -EndDate $CurrentEnd -Operations "AddedtoGroup,RemovedfromGroup" -SessionId s -SessionCommand ReturnLargeSet -ResultSize 5000 | ForEach-Object {
  ...
  Write-Progress -Activity "`
     Retrieving SharePoint Group membership changes audit from $CurrentStart to $CurrentEnd.."`
...
The output file contains $OutputEvents audit records
 ...
 {
  Write-Host `
 "The Output file available in:" -NoNewline -ForegroundColor Yellow; Write-Host "$OutputCSV"`
