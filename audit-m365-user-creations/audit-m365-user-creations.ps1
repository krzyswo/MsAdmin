...
While($true)
{
 ...
 Try
 {
  ...
  {
   Write-Host `
Audit can be retrieved only for past 90 days. Please select a date after $MaxStartDate -ForegroundColor Red
   return
  }
 }
 Catch
...
Write-Host `
Auditing Microsoft 365 user creations from $StartDate to $EndDate..." -ForegroundColor Cyan
...
{ 
 ...
 Search-UnifiedAuditLog -StartDate $CurrentStart -EndDate $CurrentEnd -Operations $Operations -SessionId s -SessionCommand ReturnLargeSet -ResultSize 5000 | ForEach-Object {
  ...
  Write-Progress -Activity "`
     Retrieving Microsoft 365 user creations from $CurrentStart to $CurrentEnd.."`
" Processed audit record count: $ProcessedAuditCount"
...
The output file contains $OutputEvents audit records
 ...
 {
  Write-Host `
 "The Output file availble in:" -NoNewline -ForegroundColor Yellow; Write-Host "$OutputCSV" `
